...
// Тренировка, "Меню шапки" → Skip workout; 05, "Пропустить тренировку"). The session becomes
// `skipped` and next week's session of the same day is generated in one transaction (049). On
// success the workout and grid queries are invalidated, so the screen re-reads the session — now
// read-only — and the grid shows the new day.
